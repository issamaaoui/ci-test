...
class Dvl_Sapbyd_Model_Soap_Order extends Dvl_Sapbyd_Model_Soap
{
    /**
    ...
    public function searchByMageID($orderId){
        ...
        $this->modelData->SalesOrderSelectionByElements->SelectionByInternalID->LowerBoundaryInternalID = $orderId;
        return $this->request();
    }
    
    public function searchByInternalID($id){
        $this->modelType = "query";
        $this->modelData->OrderSelectionByIdentification->SelectionByInternalID->LowerBoundaryInternalID = $id;
        return $this->request();
    }
    ...
    public function afterRenderRequest($xmlRequest = ""){
        $strAfterPos = '</PricingTerms>';
        foreach ($this->dataItems as $itemXml){
            $itemXml = $strAfterPos . $itemXml;
            $xmlRequest = str_replace($strAfterPos, $itemXml, $xmlRequest);
        }
        
        // Currency code for Amount and PaymentAmount
        $currencyCode = $this->modelData->SalesOrder->PricingTerms->CurrencyCode;
        $xmlRequest = str_replace('<PaymentAmount>', '<PaymentAmount currencyCode="'.$currencyCode.'">', $xmlRequest);
        $xmlRequest = str_replace('<Amount>', '<Amount currencyCode="'.$currencyCode.'">', $xmlRequest);
        
        // langageCode for FormattedName
        $websiteCode = $this->order->getStore()->getWebsite()->getCode();
        $languageCode = $this->mainConfig->website->$websiteCode->languageCode;
        $xmlRequest = str_replace('<FormattedName>', '<FormattedName languageCode="'.$languageCode.'">', $xmlRequest);

        // ref order specifique
        $xmlRequest = str_replace('</SalesOrder>', '<a02:IDcommandeMagento>'.$this->order->getIncrementId().'</a02:IDcommandeMagento></SalesOrder>', $xmlRequest);
    ...
    public function setModelData($sapbydCustomerId, $customer, $payment, $transaction){
        ...
        if ($this->mainConfig->website->$websiteCode->sap_branch == 1){ // branch case 
            ...
            $this->modelData->SalesOrder->BillToParty->Address->PostalAddress->StreetPrefixName = $orderBillingAddress->getCompany();
            $this->modelData->SalesOrder->BillToParty->Address->PostalAddress->AdditionalStreetSuffixName = $orderBillingAddress->getStreet2();
            ...
            $this->modelData->SalesOrder->AccountParty->Address->PostalAddress->AdditionalStreetSuffixName = $orderBillingAddress->getStreet2();
            $this->modelData->SalesOrder->AccountParty->Address->PostalAddress->StreetPrefixName = $orderBillingAddress->getCompany();
        ...
        $this->modelData->SalesOrder->ProductRecipientParty->Address->PostalAddress->AdditionalStreetSuffixName = $orderShippingAddress->getStreet2();
        $this->modelData->SalesOrder->ProductRecipientParty->Address->PostalAddress->StreetPrefixName = $orderShippingAddress->getCompany();
        ...
        if ($this->mainConfig->website->$websiteCode->sap_branch == 1){
            ...
            $this->modelData->SalesOrder->PaymentControl->PaymentAmount = $order->getPayment()->getBaseAmountPaid();
            ...
            $this->modelData->SalesOrder->PaymentControl->ExternalPayment->Amount = $order->getPayment()->getBaseAmountPaid();
        }
