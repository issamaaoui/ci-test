...
class Customweb_PayboxCw_ProcessController extends Customweb_PayboxCw_Controller_Action
{
	...
	public function dummyAction()
	{
		if ($this->getRequest()->isXmlHttpRequest()) {
			...
			$jsonObject['transaction_id'] = $this->getTransaction()->getTransactionId();
		...
		$this->getLayout()->getBlock('content')->append(
			$this->getLayout()->createBlock('payboxcw/dummy')
		);
	...
	public function getVisibleFieldsAction()
	{
		...
		if($payment != null){
			...
			Mage::log("PayboxCw : ProcessController::getVisibleFieldsAction() Could not find payment method '" . $_REQUEST['payment_method']);
			$html = Mage::helper("PayboxCw")->__("Technical issue: This payment methods is not available at the moment.");
		}
	...
	{
		$transaction = $this->getTransaction();
		Mage::dispatchEvent('success_paybox', array('transaction' => $transaction)); // @TODO : emplacement à vérifier - peut-être à mettre dans l'appel AJAX pour éviter une erreur sur le FRONT
