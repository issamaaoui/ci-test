 ...
 * @Bean
 */
class Customweb_PayboxCw_Model_ConfigurationAdapter implements Customweb_Payment_IConfigurationAdapter
{
	...
	public function getConfigurationValue($key, $language = null)
	{
		...
		$rs = Mage::getStoreConfig('payboxcw/general/' . $key, self::$storeId);
		...
		if (isset($multiSelectKeys[$key])) {
			...
			if (empty($rs) || $defaultValue == $rs) {
				...
				return new Customweb_Core_Stream_Input_File(Mage::getBaseDir('media') . '/payboxcw/setting/default/' . $key . '/' . $rs);
			}
		} else {
			return $rs;
		}
	...
	{
		return null != Mage::getStoreConfig('payboxcw/general/' . $key, self::$storeId);
	}

	public function getDefaultTemplateUrl()
	{
		$url = Mage::getUrl('PayboxCw/checkout/template', array(
			'_secure' => true,
			'_store' => self::$storeId,
			'_store_to_url' => true
		));
