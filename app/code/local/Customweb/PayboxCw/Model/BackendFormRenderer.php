...
class Customweb_PayboxCw_Model_BackendFormRenderer extends Customweb_Form_Renderer
{
	private $formId = null;
	...
	public function __construct()
	{
		...
		$this->setControlCssClassResolver(Mage::getModel('payboxcw/controlCssClassResolver'));
	...
	{
		if (Mage::getModel('payboxcw/configurationAdapter')->getStoreHierarchy() == null) {
			return '';
		}
	...
	protected function renderElementScopeControl(Customweb_Form_IElement $element)
	{
		...
		$output .= '<label for="' . $scopeControlId . '">' . Customweb_I18n_Translation::__('Use Default') . '</label>';
		return $output;
	}
	...
	public function renderElementsJavaScript(array $elements)
	{
		...
		$js .= "\n";
		$js .= $this->renderValidatorCallbacks($visibleElements);
		$js .= $this->renderOnLoadJs(array('registerValidatorCallbacks'));
		return $js;
	}
