...
class Customweb_PayboxCw_Model_Api2_Transaction_Rest extends Customweb_PayboxCw_Model_Api2_Transaction
{
	...
	{
		$transaction = Mage::getModel('payboxcw/transaction')->load($id);
	...
	protected function prepareEntry(Customweb_PayboxCw_Model_Transaction $transaction)
	{
		...
		if (is_string($transaction->getTransactionObject())) {
			$transaction->setTransactionObject(Mage::helper('PayboxCw')->unserialize($transaction->getTransactionObject()));
		}
