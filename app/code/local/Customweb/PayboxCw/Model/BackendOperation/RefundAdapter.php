 ...
 * @Bean
 */
class Customweb_PayboxCw_Model_BackendOperation_RefundAdapter implements Customweb_Payment_BackendOperation_Adapter_Shop_IRefund
{
	...
	protected function createCreditmemo(Mage_Sales_Model_Order $order, $data)
	{
		...
		if ($creditmemo) {
			if (($creditmemo->getGrandTotal() <=0) && (!$creditmemo->getAllowZeroGrandTotal())) {
				Mage::throwException(
					Mage::helper('PayboxCw')->__("Credit memo's total must be positive.")
				);
			}

			Mage::register('cw_payboxcw_refund_update', true);
