...
class Customweb_Payment_Authorization_Method_CreditCard_ElementBuilder {
	...
	protected function buildCardHolderElement() {
		...
		if ($this->getCardHolderFieldName() !== null) {
			...
			if ($this->isFixedCardHolderActive()) {
				$this->cardHolderControl = new Customweb_Form_Control_Html($this->getCardHolderFieldName(), $this->getCardHolderName());
			}
			else {
				$this->cardHolderControl = new Customweb_Form_Control_TextInput($this->getCardHolderFieldName(), $this->getCardHolderName());
	...
	protected function buildExpiryElements() {
		...
		if ($this->isFixedCardExpiryActive()) {
			$expiryData = "<span class='fixed-expriy-month'>" . $this->getSelectedExpiryMonth() . '</span>';
			$expiryData .= "/<span class='fixed-expriy-year'>" . $this->getSelectedExpiryYear() . '</span>';
